...
final class AdvanceDayHandlerTest extends TestCase
{
    ...
    public function testDailyKpiIncludesSameDayMigrationEvents(): void
    {
        $world = new World('seed-1');
        ...
        $migrationEvents = array_values(array_filter(
            $persisted,
            static fn (object $entity): bool => $entity instanceof CharacterEvent && $entity->getType() === 'settlement_migration_committed',
        ));

        $kpiRows = array_values(array_filter(
            $persisted,
            static fn (object $entity): bool => $entity instanceof \App\Entity\SimulationDailyKpi,
        ));

        self::assertNotSame([], $migrationEvents);
        self::assertCount(1, $kpiRows);
        /** @var \App\Entity\SimulationDailyKpi $kpi */
        $kpi = $kpiRows[0];
        self::assertSame(count($migrationEvents), $kpi->getMigrationCommits());
    }
